module.exports = $importAll([
    ...
    return Lexer.setup({
        ...
        tokenPatterns: [
            [Regex.from(/'(\\.|.)'/iy), text => ({
                id: Tokens.constantChar,
                value: text.length === 3 ? text.charCodeAt(1) : text.charCodeAt(2)
            })],
            [Regex.from(/\d+\.\d/iy), text => ({
                id: Tokens.constantFloat,
                value: Float.fromString(text).withDefault(0.0)
            })],
